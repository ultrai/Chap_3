...
% 1.8818    2.6122    3.5774 with ntrees 1
% 1.7198    2.6274    3.1627 with ntrees 8 with 1e5
%  1.6722    2.7630    3.0461 with ntrees 8 with 2e5
%  1.6722    2.7630    3.0461 with ntrees 16 with 2e5 yep it didnt work out
%  37.3125    5.6484   53.1612 with 4 stats feat 
% 26.3421    4.6088   53.9453 with 2 stats feat 
dfs={'imWidth',32, 'gtWidth',16, 'nPos',1e6, 'nNeg',1.5e6, 'nImgs',inf, ...
  ...
  'simSmooth',8, 'normRad',4, 'shrink',1, 'nCells',8, 'rgbd',0, ...
  'stride',2, 'multiscale',1, 'sharpen',1, 'nTreesEval',4, ...
  'nThreads',4, 'nms',0, 'seed',1, 'useParfor',0, 'modelDir','models/', ...
  'modelFnm','models', 'bsdsDir','Data/','nChnsColor',3};
...
nChnsGrad=(opts.nOrients+1)*2; nChnsColor=opts.nChnsColor;
if(opts.rgbd==1), nChnsColor=1; end
if(opts.rgbd==2), nChnsGrad=nChnsGrad*2; nChnsColor=nChnsColor+1; end
nChns = nChnsGrad*nChnsColor+1; opts.nChns = nChns;
...
for i = 1:nImgs
  ...
  I=load([trnImgDir imgIds{i} '.' ext]); I=I.Img;siz=size(I);
